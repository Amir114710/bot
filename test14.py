...
def fetch_prices(count=30):
    """ قیمت‌های جدید را دریافت می‌کند. """
    new_prices = []
    for _ in range(count):
        price = fetch_price()
        if price is not None:
            new_prices.append(price)
        time.sleep(1)  # کمی توقف بین درخواست‌ها
    return new_prices
...
def generate_signals():
    global position
    while True:
        prices = fetch_prices()  # دریافت ۳۰ قیمت جدید
        if len(prices) >= 30:
        ...
        time.sleep(3600)  # خوابیدن برای یک ساعت
